...
# **$R_{\text{on}}/g_m$ Based Design methodology for Dynamic Amplifiers**

### A Proof of Concept: $R_{\text{on}}/g_m$ Based Design Methodology for Dynamic Amplifiers
...
---
 ...
 # **Abstract**
 ---
 This work presents an $R_{\text{on}}/g_m$ based design methodology for Inverter based dynamic amplifiers(IBA), addressing a fundamental gap in existing approaches where the large-signal RC settling phase governed by the final stage device ON resistance $R_{\text{on}}$ remains uncharacterized until post-simulation. Unlike the conventional $g_m/I_D$ methodology, which targets only the small-signal transconductance, the proposed approach simultaneously co-designs both settling phases through pre-characterized device look-up tables (LUTs) derived from parametric SPICE simulations in the IHP SG13G2 130 nm BiCMOS process. These LUTs take into consideration of $R_{\text{on}}/g_m$ as a function of device geometry, bias, and process corner, making worst case corner behavior and valid bias deadzone boundaries directly readable at the design entry stage without iterative simulation. A head to head comparison with the $g_m/I_D$ methodology confirms that the $R_{\text{on}}/g_m$ approach achieves equivalent settling accuracy while substantially reducing design cycles and providing more useful information regarding deadzone bias requirement by surfacing process-corner sensitivity upfront.
...
* Drive a capacitive load $C_L$ to a precise output voltage $V_{\text{out}}$,
...
$$\boxed{I_{\text{out}} = g_m \cdot \Delta V_{\text{in}} }$$

then integrating that current onto the load capacitor:

$$\boxed{V_{\text{out}}(t) = \frac{1}{C_L}\int I_{\text{out}}\, dt}$$

Together these produce the familiar **exponential settling**:

$$\boxed{V_{\text{out}}(t) = V_{\text{final}}\left(1 - e^{-t/\tau}\right), \quad \tau = \frac{C_L}{g_m}}$$
...
| **Reduced voltage headroom** | Lower $V_{\text{DD}}$ at scaled nodes | Restricted output swing |
...
Consider an inverter (a PMOS and NMOS in series) driving a capacitive load. If the input is near the trip point $V_{\text{tp}}$:
...
The transition from large-signal to small-signal settling is governed by the **deadzone voltages** $V_{\text{DZP}}$ and $V_{\text{DZN}}$. These are the gate bias voltages applied to the output-stage PMOS and NMOS respectively, chosen such that both transistors sit at the edge of saturation with balanced drain currents, so that the net output current $I_{\text{out}} = 0$ and the output impedance $R_{\text{out}}$ is maximised. This defines the **deadzone**: the input-referred window around which the amplifier behaves as a high-gain small-signal stage with a dominant output pole.

$$\boxed{I_{\text{out}} = 0, \quad R_{\text{out}} \to \text{max} \quad \Longleftrightarrow \quad V_{\text{in}} \in [V_{\text{DZN}},\, V_{\text{DZP}}]}$$
...
> In the IBA(Inverter based amplifier), the deadzone arises implicitly from the inverter trip point, the same node sets both the large-signal drive and the small-signal bias, leaving no independent handle on $V_{\text{DZP}}/V_{\text{DZN}}$. The Ring Amplifier resolves this by using dedicated driver stages to set the deadzone voltages explicitly and independently.
...
*Figure 4: Animation of the transient response of the Inverter-Based Amplifier (IBA) with step input $V_X$ and deadzone voltages $V_{\text{DZP}}$ (0.4 V) and $V_{\text{DZN}}$ (0.4 V). $V_{\text{out}}$ and $I_{\text{out}}$ are shown during the linear and nonlinear settling phases of the dynamic amplifier (behavioral model).*
...
$$
\epsilon_{\text{dynamic}} = \frac{V_{\text{out}}(t) - V_{out,\text{final}}}{V_{out,\text{final}}}
$$
...
* **Ring Amplifiers (RAMPs)** - a cascaded ring of inverter-like stages with controlled deadzone bias voltages $V_{\text{GP}}$, $V_{\text{GN}}$ to set the small-signal settling window **[2]**.
...
* **Initial RC settling** - for a large-signal step input, the output of the second stage ($V_{\text{GP}}$) is driven close to rail-to-rail. This places a large overdrive on the output-stage PMOS, whose gate-source voltage remains approximately fixed while the drain-source voltage changes. The output current during this phase is governed by the $R_{\text{ON}}$ of the output-stage PMOS, charging the load capacitance with an impulse-like current.

* **Large-signal settling** - as feedback from the output reaches back to $V_{\text{GP}}$, the gate-source voltage begins to change while the drain-source voltage has nearly settled. The output current during this phase is a function of the large-signal $G_m$ of the ring amplifier, determined by the combined gain of the first two stages and the non-linear $G_m$ of the output stage.

* **Small-signal settling** - as $V_{\text{out}}$ approaches its final settling value, $V_x$ settles to the optimal bias point where PMOS and NMOS currents balance. The net output current driving the load capacitor goes to zero and the circuit enters small-signal operation, with a quiescent current $I_Q$ flowing through both output-stage devices. The settling during this phase is governed by the small-signal $g_m$.
...
The methodology proceeds as follows. A **unit inverter** is first designed shared across stages $A_1$, $A_2$, and $A_3$ with $L_{\text{NMOS}} = L_{\text{min}}$ for $A_1$ and $A_2$, and a longer $L$ for $A_3$ to ensure large output impedance. The NMOS width is initialized to a manually chosen reasonable value, $V_{\text{DD}}$ is fixed manually, and a target $V_{\text{cm}}$ is set. This unit inverter is then replicated with multipliers to scale the stages.

Two test conditions drive the optimization loop:

* **Test (A):** $V_{\text{in}}$ and $V_{\text{out}}$ are both shorted and forced to the trip point and held fixed. The small-signal parameters of the ring amplifier are extracted and checked against user-specified targets (gain, bandwidth, phase margin).
* **Test (B):** Only $V_{\text{in}} = V_{\text{cm}}$ is applied. The optimizer checks whether $V_{\text{out}} = V_{\text{cm}}$, confirming that the amplifier is biased at the desired common-mode operating point.
...
<em>Fig. 7: Test conditions used in the Conrad et al. optimizer-based Ring Amplifier design methodology [4]. (A) Both</em> $V_{\text{in}}$ <em>and</em> $V_{\text{out}}$ <em>shorted to trip point for small-signal parameter extraction. (B) Only</em> $V_{\text{in}} = V_{\text{cm}}$ <em>applied to verify common-mode output. (C) Maximum slew current evaluation to account for</em> $I_{\text{max},P} \neq I_{\text{max},N}$ <em>asymmetry.</em>

The cost function sweeps $V_{\text{cm}}$, $V_{\text{DZP}}$, $V_{\text{DZN}}$ (with $V_{\text{DZP}} = V_{\text{DZN}}$), and the widths of the PMOS ($M_P$) and NMOS ($M_N$), checking whether the resulting sizes are close to an integer multiplier of the unit cell and whether the small-signal parameters at Test (A) match the user specifications. The candidate sizes are then applied to Test (B): if $V_{\text{out}} = V_{\text{cm}}$ is satisfied, the sizes are accepted; otherwise the optimizer iterates. A third condition **(C)** additionally evaluates the maximum slew current of the PMOS and NMOS output devices and accounts for the asymmetry when $I_{\text{max},P} \neq I_{\text{max},N}$.

The key limitation of this approach is that the entire methodology is fundamentally iterative cycling through three test conditions, sweeping parameters, and parsing simulation data without any direct connection to the underlying three dynamic settling phases. The optimizer treats the ring amplifier as a black box: it checks whether small-signal parameters pass a cost function, but provides no pre-simulation insight into $R_{\text{ON}}$, the RC settling phase, the large-signal $G_m$ phase, or how these behave across process corners. Critically, even after the optimizer converges on a solution, the designer must still run and iterate on full transient simulations to verify actual settling performance, the same manual iterative loop that existed before, just with an automated parameter sweep layer on top. The root problem remains unsolved: there is no quantitative, first-principles handle that connects transistor sizing decisions directly to dynamic settling behavior before simulation.

**Our work addresses this gap.** Rather than relying on an iterative simulation-based optimizer that is dependent on an empirical starting point, we propose a **pre-simulation design methodology** using the **$R_{\text{on}}/g_m$** parameter. This approach gives the designer quantitative insight into both the RC settling phase (governed by $R_{\text{on}}$) and the small-signal settling phase (governed by $g_m$), **enabling corner-aware transistor sizing directly from lookup tables *without any iterative SPICE runs***. The $R_{\text{on}}/g_m$ curves provides a starting point that [4] had to approximate, and in doing so rendering the optimizer loop itself unnecessary for the sizing step.

---



## 2.3 $R_{\text{on}}/g_m$ Based Design Methodology
...
During the initial transient, an impulse current drives $V_{\text{out}}$ rapidly toward its Final Voltage ($V_{\text{out},\text{final}}$) by charging the load capacitor through the transistor's on-resistance $R_{\text{on}}$. This is modeled as **RC-type settling**, where R is signal-dependent.

- The current during this phase is dictated by the **on-resistance** ($R_{\text{on}}$) of the transistor. The $R_{\text{on}}$ is Non-Linear in nature due to its signal dependance.
- This phase is critical for fast initial movement of $V_{\text{out}}$ toward the final output voltage.
...
- The Variation of $R_{\text{on}}$ across corner isn't captured by the $g_m/I_D$ methodology which can silently degrade Slow-corner settling.

> **Key insight:** A circuit with well-sized $g_m$ can still exhibit slow settling if $R_{\text{on}}$ is not explicitly constrained, this failure is invisible in a $g_m/I_D$ based flow.

To overcome this, we propose using the **$R_{\text{on}}/g_m$** ratio as a design-driven metric.

- The proposed **$R_{\text{on}}/g_m$** based methodology should provide information about the required input voltage step.
- The proposed **$R_{\text{on}}/g_m$** should provide additional information about Non-linear settling and DC bias(Deadzone ranges).

---
### The $R_{\text{on}}/g_m$ Metric

The ratio $R_{\text{on}}/g_m$ simultaneously captures:

- **Large-signal settling speed** via $R_{\text{on}}$
...
### $R_{\text{on}}/g_m$ Based Design Flow
...
#### **Step 2**: Pick $R_{\text{on}}$ First

- Decide what **fraction of total settling time** $T_{\text{settle}}$ is allocated to the **non-linear (large-signal)** phase vs. the **small-signal (linear)** phase. This split is the key design knob. Here $\alpha$ denotes the fraction reserved for the small-signal phase, so $(1-\alpha)$ is the non-linear budget.

- The empirical time constants are:
  - Non-linear phase: $\tau_{\text{LS}} = R_{\text{on}} C_{\text{load}}$ → set by the switch resistance
  - Small-signal phase: $\tau_{\text{SS}} = C_{\text{load}}/g_m$ → set by the transconductance

- **Size $R_{\text{on}}$** from the non-linear settling budget:

$$R_{\text{on}} = \frac{(1-\alpha) \cdot T_{\text{settle}}}{5 \cdot C_{\text{load}}}$$

  where $(1-\alpha)$ is the time fraction allocated to non-linear settling (factor of 5 for ~99% settling in that phase). The required $R_{\text{on}}$ for a given $C_{\text{load}}$ then determines the **width scaling multiplier $M$**.

- **Size $g_m$** to satisfy **both** constraints — take the larger of the two:
  - From small-signal BW spec (application requirement): $g_{m,\text{BW}} = 2\pi\,f_{\text{BW}}\,C_{\text{load}}$
  - From settling budget (hyper-optimized lower bound): $g_{m,\text{min}} = \dfrac{5\,C_{\text{load}}}{\alpha \cdot T_{\text{settle}}}$

$$g_m = \max\,(g_{m,\text{min}},\; g_{m,\text{BW}})$$

  Designing only for $\alpha \cdot T_{\text{settle}}$ gives the minimum $g_m$ that just completes settling (hyper-optimized), but most applications impose a small-signal BW requirement that demands a larger $g_m$. The final $g_m$ is set by whichever constraint is tighter.

  > *Example*: $T_{\text{settle}} = 1\,\mu\text{s}$, $\alpha = 0.5$, $C_{\text{load}} = 1\,\text{pF}$, $f_{\text{BW}} = 10\,\text{MHz}$.
  > $R_{\text{on}} = \frac{0.5\,\mu\text{s}}{5 \times 1\,\text{pF}} = 100\,\text{k}\Omega$.
  > $g_{m,\text{min}} = \frac{5 \times 1\,\text{pF}}{0.5\,\mu\text{s}} = 10\,\mu\text{S}$, $\quad g_{m,\text{BW}} = 2\pi \times 10\,\text{MHz} \times 1\,\text{pF} \approx 63\,\mu\text{S}$ → use $63\,\mu\text{S}$.
...
- Scaling the unit cell by multiplier (M) gives:
  - $R_{\text{on}} \rightarrow R_{\text{unit}}/M$
  - $g_m \rightarrow g_{m,\text{unit}} \times M$
  - $I_D \rightarrow I_{D,\text{unit}} \times M$

#### **Step 4**: Tune $g_m$ via the Replica Path if needed

- The *replica path* which is used to Bias the signal path devices uses:
  - $R_{\text{unit}}$, $I_{\text{unit}}$, $W_{\text{unit}}$
  - Provides *replica tuning flexibility* through $I_{\text{unit}}$
  - $g_m$ tuning can be achieved by adjusting $I_{\text{unit}}$ and $W_{\text{unit}}$ across corners, without changing the main signal path device width $W \times M$

This decouples $g_m$ tuning from the fixed $R_{\text{on}}$ constraint, preserving large-signal settling while correcting small-signal bandwidth.

---

### Key Insights

- **$R_{\text{on}}$ is a great design knob**, rather than a post-simulation tuning.
- With fixed $g_m/I_D$, $R_{\text{on}}$ can be significantly worse in the slow corner affecting large-signal settling even when $g_m$ is nominally correct.
- The same $g_m$ can be achieved across corners in subthreshold with fixed $I_D$, but $R_{\text{on}}$ will diverge, this is only visible with the $R_{\text{on}}/g_m$ metric.
...
- The $R_{\text{on}}/g_m$ flow enables **pre-simulation corner verification**, a capability absent from the classical $g_m/I_D$ methodology.
---
...
.control
  ...
  foreach w_val 1u 2u 3u 4u 5u 6u 7u 8u 9u 10u
      foreach vds_val 1.65 1.6 1.2375 0.825 0.4125 0.05
         print 'Simulating with W=' $w_{\text{val}} ' 'VDS=' $vds_val

         alterparam wx = $w_{\text{val}}
         ...
         if (($w_{\text{val}} eq start_w) & ($vds_val eq start_vdsx))
...
| | `foreach vds_val 1.65 1.6 ... 0.05` | Starts the inner loop, iterating the variable **`vds_val`** (drain-source voltage, $V_{\text{DS}}$). |
| **3. Parameter Updates** | `alterparam wx = $w_{\text{val}}` | **Dynamically updates** the circuit parameter `wx` with the current width value from the loop. |
| | `alterparam vdsx = $vds_val` | **Dynamically updates** the circuit parameter `vdsx` with the current $V_{\text{DS}}$ value. |
...
| **5. Compute Key Quantities** | `let Ron_unit = @n.xmn_...[vds]/@n.xmn_...[ids]` | Uses the **`let`** command to calculate **derived parameters** like the transistor's on-resistance ($R_{\text{on}}$), which is defined as $V_{\text{DS}}/I_{\text{DS}}$. |
| | `let gmron_unit = @n.xmn_...[gm]` | Calculates the transconductance ($g_m$). |
| **6. Export Results** | `wrdata path/to/output.csv ...` | **Writes** the calculated and measured quantities (voltages, currents, $R_{\text{on}}$, $g_m$, etc.) to a `.csv` file. |
...
**Sweep:** $W$ = 1-10 µm, $V_{\text{DS}}$ = 6 values, $I_{\text{bias}}$ = 5-30 µA at each ($W$, $V_{\text{DS}}$) point  
...
**Connection to the design flow:** These netlists are the simulation inputs that produce the look-up table (LUT) data for the Ron/gm methodology plots in the next section. Without this sweep, there is no data to read off $R_{\text{on}}/g_m$ vs $V_{\text{bias}}$, $g_m/I_d$, or voltage swing. Each resulting CSV represents one (L, corner) characterisation point in the design space.


### Simulations Complete - CSV Data Extracted

All 15 simulations ran successfully. Each CSV in `/content/simulation_results/` contains the swept $R_{\text{on}}$, $g_m$, $V_{\text{bias}}$, width, and length data for one (L, corner) combination.



---
# 4. Plot Generation for $R_{\text{on}}/g_m$ Methodology Based Design
---

> **Visualization of design plots used in the $R_{\text{on}}/g_m$ methodology**

In this section, we generate and analyze the plots required for circuit design using the **$R_{\text{on}}/g_m$ methodology**.  
...
1. **$V_{\text{bias}}$ vs $R_{\text{on}}/g_m$**  
2. **$V_{\text{bias}}$ vs Width**  
3. **$I_{\text{peak}}$ vs $R_{\text{on}}/g_m$**   
4. **$g_{m,\text{bias}}$ vs $R_{\text{on}}/g_m$**  
5. **$g_m/I_D$ vs $R_{\text{on}}/g_m$**  
6. **$V_{\text{swing}}$ vs $R_{\text{on}}/g_m$**


These plots form the **core design framework of the $R_{\text{on}}/g_m$ methodology**, enabling improved trade-off analysis between biasing, device sizing, current consumption, and achievable signal swing.
...
|:--:|:--|:--|:--|:--|:--|
| 1 | **$V_{\text{bias}}$ vs $R_{\text{on}}/g_m$** | $R_{\text{on}}/g_m$ (log) | Gate bias voltage $V_{\text{bias}}$ | What bias voltage do I need to hit my target $R_{\text{on}}/g_m$? | Read off $V_{\text{DZP}}$ / $V_{\text{DZN}}$ deadzone voltages directly. Corner spread shows process sensitivity. |
| 2 | **$V_{\text{bias}}$ vs Width** | Device width $W$ | Gate bias voltage $V_{\text{bias}}$ | What device width corresponds to my chosen bias point? | Use to size the unit cell $W_{\text{unit}}$ before applying the multiplier $M$. |
| 3 | **$I_{\text{peak}}$ vs $R_{\text{on}}/g_m$** | $R_{\text{on}}/g_m$ (log) | Peak current $I_{\text{peak}}$ (log) | How much peak current does the RC settling phase deliver? | Higher $I_{\text{peak}}$ → faster large-signal charging. Sets initial slew rate into $C_{\text{load}}$. |
| 4 | **$g_{m,\text{bias}}$ vs $R_{\text{on}}/g_m$** | $R_{\text{on}}/g_m$ (log) | Bias-path $g_{m,\text{bias}}$ | What is the bias-path $g_m$? | Verifies small-signal settling bandwidth: $BW = g_{m,\text{bias}} / (2\pi C_{\text{load}})$. |
| 5 | **$g_m/I_D$ vs $R_{\text{on}}/g_m$** | $R_{\text{on}}/g_m$ (log) | Efficiency $g_m/I_D$ | What power efficiency am I operating at for this $R_{\text{on}}/g_m$ choice? | Bridges the $R_{\text{on}}/g_m$ and $g_m/I_D$ methodologies - shows the efficiency cost of a given $R_{\text{on}}$ target. |
| 6 | **$V_{\text{swing}}$ vs $R_{\text{on}}/g_m$** | $R_{\text{on}}/g_m$ (log) | Output voltage swing $V_{\text{swing}}$ | What output swing is achievable at this operating point? | Confirms the chosen bias allows the required signal swing without clipping. |
...
- **Hover** over any trace to read exact ($R_{\text{on}}/g_m$, $V_{\text{bias}}$, width, $g_m/I_d$) values at any point
- **Legend** entries can be clicked to isolate individual corners or lengths
- All X-axes are $R_{\text{on}}/g_m$ (log scale), your design target goes on this axis first
- Work left-to-right through the plots: start at Plot 1 to pick $R_{\text{on}}/g_m$, then use Plots 2–6 to confirm width, current, and swing requirements


## 4.1 $V_{\text{bias}}$ vs log($\frac{R_{\text{on}}}{g_m}$)
---

The gate bias voltage $V_{\text{bias}}$ of the diode-connected replica transistor varies with device geometry and bias current across all process corners. It is set by the diode connection as $V_{\text{bias}} = V_{\text{TH}} + V_{\text{ov}}$.


The replica is diode-connected in saturation, so its bias voltage is set by $V_{\text{bias}} = V_{\text{TH}} + V_{\text{ov}}$.
In strong inversion ($g_m/I_D \lesssim 15\,\text{V}^{-1}$), the transconductance is approximated as:

$$g_m \approx \frac{2I_D}{V_{\text{ov}}} \qquad \text{(strong-inversion approximation)}$$

$$\Rightarrow \quad V_{\text{ov}} = \frac{2I_D}{g_m} \qquad \therefore \quad V_{\text{bias}} = V_{\text{TH}} + \frac{2I_D}{g_m}$$

> **Note on inversion region:** This approximation holds well for $g_m/I_D \lesssim 15\,\text{V}^{-1}$
> (moderate-to-strong inversion, corresponding to **larger $R_{\text{on}}/g_m$** values on the right
> side of the plot, where the device operates with meaningful overdrive). At higher
> $g_m/I_D > 20\,\text{V}^{-1}$ (subthreshold, **smaller $R_{\text{on}}/g_m$**, left side of plot),
> $g_m \approx I_D/nV_T$ and the formula above underestimates $V_{\text{bias}}$. At lower
> $g_m/I_D < 5\,\text{V}^{-1}$ (deep strong inversion, **largest $R_{\text{on}}/g_m$**, right
> extreme of plot), the approximation is most accurate. In all cases, $V_{\text{bias}}$ is read
> directly from the LUT plot — the formula is for physical intuition only.

Multiplying and dividing by $R_{\text{on}}$:

$$\boxed{V_{\text{bias}} = V_{\text{TH}} + \frac{2I_D}{V_{\text{swing}}} \cdot I_{\text{peak}} \cdot \frac{R_{\text{on}}}{g_m}}$$

where $R_{\text{on}} = V_{\text{swing}}/I_{\text{peak}}$. This shows that **$V_{\text{bias}}$ is linear in $R_{\text{on}}/g_m$**,
...
- **Higher $I_D$** at fixed $R_{\text{on}}/g_m$: more current demands more overdrive → $V_{\text{bias}}$ rises
- **Larger $R_{\text{on}}/g_m$** at fixed $I_D$: directly scales $V_{\text{bias}}$ upward along each trace
...
> **Reading the plot:**
> - **Top → Bottom** at fixed $R_{\text{on}}/g_m$: $I_{\text{sweep}}$ **decreases**
>   (lower current → lower overdrive required → lower $V_{\text{bias}}$)
> - **Left → Right** along a fixed $I_{\text{sweep}}$ trace: $L$ **increases**
>   (larger $R_{\text{on}}/g_m$ directly raises $V_{\text{bias}}$ through the linear relationship above)

As expected, the **SS corner** sits highest at any given $L$ and $I_{\text{sweep}}$: a slow-corner device has a larger $V_{\text{TH}}$ and lower $\mu C_{\text{ox}}$, both of which demand a higher gate voltage to source the same current. The FF corner sits lowest.

---

### Key Insight

$V_{\text{bias}}$ is the deadzone voltage, the output-stage transistor does not enter the small-signal settling region until its gate voltage crosses $V_{\text{bias}}$. The expression above makes the binding constraint explicit: **the SS corner at the largest $L$ and highest $I_{\text{sweep}}$ sets the worst-case $V_{\text{bias}}$**, which must be met by the preceding stage's output swing across all corners.

Width and current are coupled through $g_m$: increasing $W$ lowers $V_{\text{bias}}$ (through $g_m \uparrow$, so $V_{\text{ov}} \downarrow$) but also lowers $R_{\text{on}}$, directly affecting the non-linear RC settling speed. Reading this plot together with the $g_{m,\text{bias}}$ vs $R_{\text{on}}/g_m$ plot gives the complete picture, the target $R_{\text{on}}/g_m$ maps to a specific ($W$, $I_{\text{sweep}}$) pair, and the corresponding $V_{\text{bias}}$ here is the voltage that must be guaranteed across all corners for the output stage to transition into the small-signal phase.

---

## 4.2 $V_{\text{bias}}$ vs $Width$
---

The gate bias voltage $V_{\text{bias}}$ of the diode-connected replica transistor varies with device width $W$, across all bias currents $I_{\text{sweep}}$, VDS values, and process corners for a given channel length.

The diode-connected replica sets $V_{\text{bias}} = V_{\text{TH}} + V_{\text{ov}}$, where:

$$V_{\text{bias}} = V_{\text{TH}} + \sqrt{\frac{2I_D}{\mu C_{\text{ox}}(W/L)}} \qquad \Rightarrow \qquad V_{\text{bias}} - V_{\text{TH}} \propto \frac{1}{\sqrt{W}}$$

This $1/\sqrt{W}$ decay is the dominant feature of the plot, the rapid drop at small $W$ (1–5 µm) followed by a flattening at larger widths. Each trace is one combination of ($I_{\text{sweep}}$, $V_{\text{DS}}$, corner), and all follow the same curve shape, shifted vertically by their bias current.

- **Higher $I_{\text{sweep}}$** → trace shifts upward: more current demands higher overdrive to sustain it in the replica device
- **Larger $W$** → $V_{\text{bias}}$ decreases: a wider device carries the same current at a lower $V_{\text{GS}}$, reducing the required bias voltage
- **Corner spread** → SS corner (higher $V_{\text{TH}}$) sits above TT and FF at the same width and current, directly showing which corner demands the highest $V_{\text{bias}}$ to enter the small-signal settling region
- **Multiple $V_{\text{DS}}$ values overlaid** → the spread between $V_{\text{DS}}$ traces is small, confirming that $V_{\text{bias}}$ is primarily set by the diode-connected replica (which operates at $V_{\text{DS}} = V_{\text{GS}}$) and is relatively insensitive to the drain voltage of the main-path device
---
...
For a **given $W$ and bias current, what $V_{\text{bias}}$ must be applied to the gate of the final-stage transistor to ensure it operates in the small-signal settling region?**

Width and current are coupled through $g_m$: increasing $W$ lowers $V_{\text{bias}}$ but also lowers $R_{\text{on}}$, which affects the non-linear RC settling speed (Plot 3). Reading Plot 2 together with Plot 1 ($V_{\text{bias}}$ vs $R_{\text{on}}/g_m$) gives the designer the complete picture: the target $R_{\text{on}}/g_m$ from Plot 1 maps to a specific ($W$, $I_{\text{sweep}}$) combination here, and the corresponding $V_{\text{bias}}$ is the deadzone voltage that must be met across all process corners for the IBA final stage to enter small-signal mode.

---

## 4.3 Log($I_{\text{peak}}$) vs Log($\frac{R_{\text{on}}}{g_m}$)
---

**$I_{\text{peak}}$** is the initial large-signal current that flows through the output-stage transistor at the onset of the settling transient. This current charges the load capacitor $C_L$ during the non-linear RC phase it determines how fast the output moves toward its final value before the small-signal exponential phase takes over.

In the linear (triode) region, the output-stage transistor on-resistance is:
$$R_{\text{on}} = \frac{L}{\mu C_{\text{ox}} W V_{\text{ov}}} \propto \frac{L}{W I_D}$$

Combining these:
$$\boxed{I_{\text{peak}} = \frac{V_{\text{swing}}}{R_{\text{on}}} \propto \frac{1}{R_{\text{on}}/g_m}}$$

On a log-log scale this gives:
$$\log(I_{\text{peak}}) = -\log\!\left(\frac{R_{\text{on}}}{g_m}\right) + \text{const}$$

which is approximately a **straight line with slope $-1$** seen in the plot. The spread between TT/SS/FF traces at each bias current reflects the process-corner sensitivity of $I_{\text{peak}}$ - the FF corner (green) consistently delivers higher $I_{\text{peak}}$ than SS (red) at the same $R_{\text{on}}/g_m$, since a faster process has lower $R_{\text{on}}$ and higher carrier mobility, both of which increase the initial charging current. This corner spread grows at larger $R_{\text{on}}/g_m$ values (longer $L$ or lower $I_D$), making worst-case SS-corner $I_{\text{peak}}$ the binding constraint when sizing for non-linear settling speed.
...
> **Reading the plot:**
> - **Top → Bottom** at fixed $R_{\text{on}}/g_m$: $I_{\text{sweep}}$ **decreases**  
>   (lower current → lower $I_{\text{peak}}$)
>
> - **Left → Right** along a fixed $I_{\text{sweep}}$ trace: $R_{\text{on}}/g_m$ **increases**  
>   ($L \uparrow$ or $W \downarrow$ → $R_{\text{on}} \uparrow$ → $I_{\text{peak}} = V_{\text{swing}}/R_{\text{on}} \downarrow$ → slower initial charging)

---

- **Moving Right ($R_{\text{on}}/g_m \uparrow$):**  
  $R_{\text{on}} \uparrow$ → $I_{\text{peak}} \downarrow$ → slower non-linear settling  

- **Moving Left ($R_{\text{on}}/g_m \downarrow$):**  
  $R_{\text{on}} \downarrow$ → $I_{\text{peak}} \uparrow$ → faster initial charging  

- **Each trace = fixed $I_{\text{sweep}}$:**  
  Higher $I_{\text{sweep}}$ shifts curves upward (higher $I_{\text{peak}}$)

- **Corners:**  
  **SS** sits lowest (higher $R_{\text{on}}$ → lower $I_{\text{peak}}$),  
  **FF** sits highest (lower $R_{\text{on}}$ → higher $I_{\text{peak}}$)
---

### Key Insight

The geometry dependence of $R_{\text{on}}/g_m$ is explicit:

$$\frac{R_{\text{on}}}{g_m} \propto \frac{L^2}{W I_D}$$

This means $L$ is the dominant geometry knob for the non-linear, but the replica bias path allows $I_D$ to be adjusted independently tuning $g_m$ and small-signal bandwidth without disturbing the main-path $R_{\text{on}}$. This is the pre-simulation design handle that the $g_m/I_D$ methodology does not provide.

---

## 4.4 $g_{m,\text{bias}}$ vs Log($R_{\text{on}}/g_m$)
---

**$g_{m,\text{bias}}$** is the transconductance of the transistor in the replica bias branch, the same device as the unit output-stage transistor. It sets the small-signal settling bandwidth and appears as the $g_m$ denominator in the $R_{\text{on}}/g_m$ design parameter. Because the bias branch mirrors the output transistor, tuning $I_{\text{sweep}}$ directly controls $g_{m,\text{bias}}$ without disturbing the main signal path.

These two quantities are evaluated at **different operating points**: $R_{\text{on}}$ is extracted with the switch fully on in triode ($V_{\text{GS}} = V_{\text{DD}}$, $V_{\text{DS}} = 50\,\text{mV}$), while $g_{m,\text{bias}}$ is evaluated in saturation at the replica bias current a different overdrive from $V_{GS,\text{sw}} - V_{\text{TH}}$:

$$R_{\text{on}} = \frac{L}{\mu C_{\text{ox}} W (V_{GS,\text{sw}} - V_{\text{TH}})}, \qquad g_{m,\text{bias}} = \sqrt{2\mu C_{\text{ox}}\frac{W}{L}I_D} = \frac{2I_D}{V_{ov,\text{bias}}}$$

Because $V_{ov,\text{bias}} \neq V_{GS,\text{sw}} - V_{\text{TH}}$, the product $R_{\text{on}} \cdot g_{m,\text{bias}}$ is **not constant**. Eliminating $L$ at fixed $W$ and $I_D$, using $R_{\text{on}} \propto L$, $g_{m,\text{bias}} \propto L^{-1/2}$, so $R_{\text{on}}/g_m \propto L^{3/2}$ gives the fundamental scaling:

$$g_{m,\text{bias}} \propto \left(\frac{R_{\text{on}}}{g_m}\right)^{-1/3}$$
...
- **Higher $I_{\text{sweep}}$** → curve shifts upward: more bias current raises $g_{m,\text{bias}} = 2I_D/V_{\text{ov}}$ at every $R_{\text{on}}/g_m$ value ($g_{m,\text{bias}} \propto \sqrt{I_D}$, so doubling $I_{\text{sweep}}$ lifts the curve by $\sqrt{2}$)
- **Left → Right** along a fixed $I_{\text{sweep}}$ trace: $L$ increases ($R_{\text{on}}/g_m \propto L^{3/2}$ pushes the operating point rightward while $g_{m,\text{bias}} \propto L^{-1/2}$ pulls it downward)
- **Corner spread** → process corners shift $\mu C_{\text{ox}}$ and $V_{\text{TH}}$, vertically separating the constant $I_D$ curves; the SS corner sits highest at any given $(W, L, I_D)$
---
...
The same $-1/3$ exponent holds whether $W$ or $L$ is varied any $(W, L)$ pair mapping to the same $R_{\text{on}}/g_m$ value at the same $I_D$ produces the same $g_{m,\text{bias}}$. The traces for different lengths therefore do not form separate curves; each length accesses a **different segment of the same universal curve**, and all collapse onto a single locus in the $(R_{\text{on}}/g_m,\, g_{m,\text{bias}})$ plane.

> **Reading the plot:**
> - **Top → Bottom** at fixed $R_{\text{on}}/g_m$: $I_{\text{sweep}}$ **decreases**
>   (lower bias current → lower $g_{m,\text{bias}} = 2I_D/V_{ov,\text{bias}}$)
> - **Left → Right** along a fixed $I_{\text{sweep}}$ trace: $L$ **increases**
>   ($R_{\text{on}}/g_m \propto L^{3/2}$ grows rapidly, extending the trace rightward and downward along the same universal curve)

---

### Key Insight

**Given a target small-signal time constant $\tau_{\text{ss}} = C_L / g_{m,\text{bias}}$, what bias current is required without committing to a specific $(W, L)$?**

Because $g_{m,\text{bias}}$ is uniquely determined by $R_{\text{on}}/g_m$ and $I_D$ alone, the required $I_{\text{sweep}}$ can be read directly from this plot once $\tau_{\text{ss}}$ is specified. Width is then chosen independently to meet the $R_{\text{on}}/g_m$ target (non-linear phase speed), completing the two-axis co-design that $g_m/I_D$ alone cannot decouple.

---

## 4.5 $g_m/I_D$ vs Log($\frac{R_{\text{on}}}{g_m}$)
---

- Here we look at the $g_m/I_D$ of the transistor as a function of the $R_{\text{on}}/g_m$ design metric, sweeping bias current $I_{\text{sweep}}$ across process corners.

- We can see that $g_m/I_D$ decreases as $R_{\text{on}}/g_m$ increases

- $R_{\text{on}}/g_m \propto L^2/WI_D$ and $g_m/I_D = 2/V_{\text{ov}}$ as $R_{\text{on}}/g_m$ increases (wider device, lower $I_D$, or longer $L$), the overdrive $V_{\text{ov}}$ increases, pushing the device deeper into strong inversion and reducing transconductance efficiency:

$$\frac{g_m}{I_D} = \frac{2}{V_{\text{ov}}} \propto \frac{1}{\sqrt{R_{\text{on}}/g_m \cdot I_D}}$$
...
The bias current is the primary design knob in this plot. Each trace corresponds to a fixed $I_{\text{sweep}}$:

- **Higher $I_{\text{sweep}}$** → trace shifts **downward** at the same $R_{\text{on}}/g_m$,  more current for the same geometry means the device operates deeper into strong inversion, reducing $g_m/I_D$. The $R_{\text{on}}/g_m$ operating range of the trace remains the same; only the efficiency drops.
- **Lower $I_{\text{sweep}}$** → trace shifts **upward** at the same $R_{\text{on}}/g_m$, less current keeps the device closer to moderate inversion, improving $g_m/I_D$ while the RC settling speed (set by geometry) is unchanged.

For a given geometry ($W$, $L$), the designer chooses $I_{\text{sweep}}$ to trade between transconductance efficiency and settling speed, this trade-off is directly readable from the plot.

This plot **connects the $R_{\text{on}}/g_m$ and $g_m/I_D$ methodologies**, and the relationship is strictly one-directional:

> **From $R_{\text{on}}/g_m$ → you can read $g_m/I_D$ directly off this plot.**
> **From $g_m/I_D$ → you cannot recover $R_{\text{on}}/g_m$ without additional simulation.**

The reason is fundamental: $g_m/I_D$ characterises only the small-signal operating point. It contains no information about $R_{\text{on}}$, which depends separately on $V_{\text{ov}}$, $W$, $L$, and the large-signal drain voltage. A designer entering from $g_m/I_D$ can size the device for bandwidth, but has no pre-simulation handle on where that size sits on the $R_{\text{on}}/g_m$ axis and therefore no visibility into the non-linear RC settling phase.

Entering from $R_{\text{on}}/g_m$ subsumes the $g_m/I_D$ design entirely: once $R_{\text{on}}/g_m$ and $I_{\text{sweep}}$ are chosen, this plot immediately tells the designer what $g_m/I_D$ and therefore what inversion region and power efficiency corresponds to that operating point. **The $R_{\text{on}}/g_m$ methodology is therefore a strict superset of the $g_m/I_D$ methodology for dynamic amplifier design.**
...
A designer entering from $g_m/I_D$ can size the device for bandwidth, but has no pre-simulation visibility into Non-Linear settling information ($R_{\text{on}}$), the deadzone bias voltages, or worst-case corner behaviour all of which require additional SPICE runs to discover. Entering from $R_{\text{on}}/g_m$ subsumes this entirely: once $R_{\text{on}}/g_m$ and $I_{\text{sweep}}$ are fixed, $g_m/I_D$ is directly readable from this plot and both settling phases are co-designed in a single step. The $g_m/I_D$ axis is a projection of the $R_{\text{on}}/g_m$ design space onto a single dimension, every $g_m/I_D$ point maps to a unique $R_{\text{on}}/g_m$ point, but the reverse does not exist without simulation.

---

## 4.6 $V_{\text{swing}}$ vs Log($\frac{R_{\text{on}}}{g_m}$)
---

**$V_{\text{swing}}$** is defined as:

$$V_{\text{swing}} = |v(VG) - v(Vbias)| = v(VG) - v(Vbias) \quad \text{(when } v(VG) > v(Vbias)\text{)}$$

It represents the gate drive headroom beyond the quiescent bias point the voltage the gate must traverse before the output-stage transistor enters the small-signal settling region. During non-linear settling, the dynamic amplifier sees an initial large-signal gate swing close to $V_{\text{DD}}$; as the output approaches its final value and the gate voltages of the final-stage devices reach $V_{\text{GP}} = V_{\text{bias},P}$ and $V_{\text{GN}} = V_{\text{bias},N}$, $V_{\text{swing}}$ equals zero, at which point $I_{\text{out}} = 0$ and the circuit transitions to small-signal mode.

In the triode region, $R_{\text{on}}$ depends on overdrive voltage, which itself depends on how far the gate is driven above the bias point:

$$R_{\text{on}} = \frac{L}{\mu C_{\text{ox}} W (v(VG) - V_{\text{th}})} \qquad \Rightarrow \qquad R_{\text{on}} \downarrow \text{ as } v(VG) \uparrow$$

A larger $V_{\text{swing}} = v(VG) - v(Vbias)$ means the transistor is driven harder into the triode region resulting in lower $R_{\text{on}}$, faster RC settling. This is why the plot shows a **negative slope**: larger $V_{\text{swing}}$ corresponds to smaller $R_{\text{on}}/g_m$.

$$\boxed{V_{\text{swing}} = v(VG) - v(Vbias) \propto \frac{1}{\sqrt{R_{\text{on}}/g_m}}}$$

At $v(VG) = V_{\text{DD}} = 1.65\,\text{V}$ (the fully-on condition used in earlier design plots), this simplifies to:

$$V_{\text{swing}} = V_{\text{DD}} - v(Vbias)$$
...
A common assumption in dynamic amplifier design is that the output-stage transistor always sees a full-rail swing ($V_{\text{swing}} \approx V_{\text{DD}}$) during the non-linear phase. **This is not always true.**

Consider a designer who targets a small $R_{\text{on}}/g_m$ expecting fast non-linear settling. From Plot 4.3, small $R_{\text{on}}/g_m$ implies large $I_{\text{peak}}$ and fast RC charging. But Plot 4.6 reveals the other side: **a small $R_{\text{on}}/g_m$ is only achievable if the input swing is large enough to drive the gate far above $v(Vbias)$**. If the actual input signal never delivers that swing for example, in a low-voltage or small-signal application the transistor never reaches the operating point the designer assumed, and the expected settling speed is not achieved.

Conversely, at large $R_{\text{on}}/g_m$ (right side of the plot), $V_{\text{swing}}$ is small, the gate barely rises above $v(Vbias)$, the transistor stays near the edge of the linear region, and the non-linear RC phase contributes little to nothing in the overall settling. The small-signal phase dominates.

This plot therefore answers: **for a given $R_{\text{on}}/g_m$ target, what input swing must the circuit actually see to operate at that point?**
...
> **Reading the plot:**
> - **Top → Bottom** at fixed $R_{\text{on}}/g_m$: $I_{\text{sweep}}$ **increases**  
>   (more bias current → higher $v(Vbias)$ → less headroom → smaller $V_{\text{swing}}$)
>
> - **Left → Right** along a fixed $I_{\text{sweep}}$ trace: $R_{\text{on}}/g_m$ **increases**  
>   ($L \uparrow$ or $W \downarrow$ → $R_{\text{on}} \uparrow$ → device driven less hard → $V_{\text{swing}} \downarrow$)

---

- **Moving Left ($R_{\text{on}}/g_m \downarrow$):**  
  $V_{\text{swing}} \uparrow$ - gate is driven hard above $v(Vbias)$, transistor deep in triode, $R_{\text{on}}$ low, fast non-linear settling. Only achievable if the input signal delivers sufficient swing.

- **Moving Right ($R_{\text{on}}/g_m \uparrow$):**  
  $V_{\text{swing}} \downarrow$ - gate barely clears $v(Vbias)$, non-linear phase contributes little, small-signal settling dominates.

- **Higher $I_{\text{sweep}}$** raises $v(Vbias)$ (more current needs more gate drive), reducing $V_{\text{swing}}$ at the same geometry, traces shift downward.

- **Longer $L$** pushes traces rightward (higher $R_{\text{on}}/g_m \propto L^2$) with lower $V_{\text{swing}}$ at the same $I_{\text{sweep}}$.

- **Corners:** SS corner gives lowest $V_{\text{swing}}$ at the same $R_{\text{on}}/g_m$ and higher $V_{\text{th}}$ raises $v(Vbias)$, eating into the available headroom. This is the worst-case corner for non-linear settling speed, consistent with Plot 4.1.

---

### Key Insight

$V_{\text{swing}}$ is not a free parameter; it is set by the application's input signal amplitude and the bias point simultaneously. This plot makes visible a constraint that the $g_m/I_D$ methodology cannot surface: **there is a minimum required input swing to operate at any given $R_{\text{on}}/g_m$ target**. A designer who picks a small $R_{\text{on}}/g_m$ for fast settling must verify that the circuit actually sees the corresponding $V_{\text{swing}}$; otherwise the assumed operating point is never reached and the settling speed budget is violated. Reading Plot 4.6 alongside Plot 4.1 ($V_{\text{bias}}$ vs $R_{\text{on}}/g_m$) and Plot 4.3 ($I_{\text{peak}}$ vs $R_{\text{on}}/g_m$) gives the complete pre-simulation picture of the non-linear settling phase.
...
Let's compare the performance of an inverter-based amplifier (IBA), which is inherently a dynamic amplifier, designed using traditional $g_m/I_D$ and $R_{\text{on}}/g_m$ based methodologies and evaluate which methodology provides more useful design insights into dynamic amplifiers and reduces the design iteration count required to reach the final specification.
...
|---|---|---|---|
| Supply voltage | $V_{\text{DD}}$ | 1.65 V | IHP SG13G2 nominal |
| Settling time | $T_{\text{settle}}$ | 250 ns | 5$\tau$, 99.3% settled |
| Settling time constant | $\tau_{\text{cl}}$ | 50 ns | $T_{\text{settle}}/5$ |
| Closed-loop bandwidth | $\omega_{\text{cl}}$ | 20 Mrad/s | $1/\tau_{\text{cl}}$ |
| Unity-gain bandwidth | UGB | 9.55 MHz (60 Mrad/s) | $\omega_{\text{cl}}/\beta$ |
| Target transconductance | $G_m$ | 40 µA/V | $\text{UGB} \times C_{L,\text{eff}}$ |
| DC gain | $A_0$ | 50 | $g_m/g_{\text{ds}}$ limit, lv\_nmos |
| Sampling capacitor | $C_s$ | 500 fF | |
| Feedback capacitor | $C_f$ | 250 fF | |
| Load capacitor | $C_l$ | 500 fF | |
| Effective load | $C_{L,\text{eff}}$ | 667 fF | $C_l + C_f C_s/(C_s+C_f)$ |
| Feedback factor | $\beta$ | 1/3 | $C_f/(C_s+C_f)$ |
| Closed-loop gain (ideal) | $G_{\text{cl}}$ | $-2$ V/V | $-C_s/C_f$ |
| Closed-loop gain (actual) | $G_{\text{cl}}$ | $-1.887$ V/V | Finite $A_0$ effect |
| Input bias point | $V_{\text{in},\text{dc}}$ | 825 mV | $V_{\text{DD}}/2$, inverter switching threshold |
| Input swing | $\Delta V_{\text{in}}$ | $\pm 200$ mV | $V_{\text{DD}}/2 \pm 200$ mV |

**Large-signal condition:** Input kick at virtual ground $\Delta V_x = \pm 133$ mV $> V_{\text{ov},\text{lv}\_\text{nmos}} \approx 100$ mV, guaranteeing the amplifier enters the slewing (large-signal) regime on every cycle before transitioning to exponential small-signal settling. This is the regime of interest for the $R_{\text{on}}/G_m$ methodology.
...
- For a settling time $T_{\text{settle}} = 250\,\text{ns}$ (5$\tau$), the required closed-loop bandwidth is:
  $$
  \omega_{\text{cl}} = \frac{1}{\tau_{\text{cl}}} = \frac{5}{T_{\text{settle}}} = 20\,\text{Mrad/s}, \qquad \text{UGB} = \frac{\omega_{\text{cl}}}{\beta} = \frac{20\,\text{Mrad/s}}{1/3} = 60\,\text{Mrad/s}
  $$

- The total required transconductance is:
  $$
  G_m = \text{UGB} \times C_{L,\text{eff}} = 60 \times 10^6 \times 667 \times 10^{-12} \approx 40\,\mu\text{S}
  $$

- Assume NMOS and PMOS contribute equally:
  $$
  g_{\text{mn}} = g_{\text{mp}} = \frac{G_m}{2} = 20\,\mu\text{S}
  $$

- With multiplier $m = 4$, the unit cell (replica path) must provide:
  $$
  g_{m,\text{unit}} = \frac{20\,\mu\text{S}}{4} = 5\,\mu\text{S}
  $$

- Replica path quiescent current $I_q = 0.5\,\mu\text{A}$, signal path current $= m \times I_q = 2\,\mu\text{A}$. This gives:
  $$
  \frac{g_{m,\text{unit}}}{I_q} = \frac{5\,\mu\text{S}}{0.5\,\mu\text{A}} = 10\,\frac{\text{S}}{\text{A}}
  $$
...
- Assume $g_m/g_{\text{ds}} = 50$ (intrinsic gain limit of lv\_nmos in IHP SG13G2)

---

- Using our `gmid_IHP130` model to extract device sizes:
  - Target $g_m/I_D = 10\,\text{V}^{-1}$
  - Target $g_m/g_{\text{ds}} = 50$
...
> | Slider | Value | Why |
> |:--|:--|:--|
> | **Target gm/id** | 10 | $g_{m,\text{unit}}/I_q = 5\,\mu\text{S} / 0.5\,\mu\text{A}$ |
...
> Unit cell width follows: $W_{\text{unit}} = I_q / (I_D/W)\big|_{\text{LUT}}$, then scale signal path by $m = 4$.
...
($L = 0.13\,\mu\text{m}$ → $3\,\mu\text{m}$) sweeping $V_{\text{GS}}$ at a fixed $W = 2\,\mu\text{m}$.
Each plot below captures a fundamental design trade-off used to bias the IBA core.

- **$g_m/I_D$ vs $V_{\text{GS}}$, $V_{\text{ov}}$** - identifies the inversion region and sets the DC bias point
- **$g_m/g_{\text{ds}}$ vs $g_m/I_D$** - intrinsic gain $A_{v0}$ vs transconductance efficiency trade-off
- **$I_D/W$ vs $g_m/I_D$** - current density curve used to size transistor width
- **$f_T$ vs $g_m/I_D$** - speed–power trade-off ($f_T = g_m / 2\pi C_{\text{gg}}$)
- **$C_{\text{gd}}/C_{\text{gg}}$, $C_{\text{gs}}/C_{\text{gg}}$ vs $g_m/I_D$** - parasitic capacitance partitioning and Miller coupling
...
> small-signal bandwidth requirement. The $R_{\text{on}}/g_m$ methodology uses the SPICE-simulated
> LUTs from Section 3 instead, which additionally capture $R_{\text{on}}$, deadzone bias ranges,
...
---
Given a target transconductance efficiency $g_m/I_D$ and intrinsic gain $g_m/g_{\text{ds}}$,
...
$$\mathcal{L} = w_1\!\left(\frac{g_m/I_D - \hat{x}_A}{\sigma_A}\right)^{\!2}
              + w_2\!\left(\frac{g_m/g_{\text{ds}} - \hat{x}_B}{\sigma_B}\right)^{\!2}$$
...
$\hat{x}_B = \max(g_{m}/g_{\text{ds}}\big|_{\text{target}},\, B_{\\text{min}})$ is enforced before search.
...
$$\mathcal{L} = w_1 \left(\frac{g_m/I_D - \hat{x}_A}{\sigma_A}\right)^2 + w_2 \left(\frac{g_m/g_{\text{ds}} - \hat{x}_B}{\sigma_B}\right)^2$$

**Why σ-normalise?** $g_m/I_D$ spans roughly $2$–$28\,\text{V}^{-1}$ while $g_m/g_{\text{ds}}$ spans $10$–$400$. Without normalisation, $g_m/g_{\text{ds}}$ dominates the distance simply because its numbers are larger not because it matters more. Dividing by $\sigma$ puts both terms on the same dimensionless scale, so $w_1$ and $w_2$ are true importance weights.

**The bound constraint:** $\hat{x}_B = \max(B_{\\text{min}},\, g_m/g_{\text{ds}}\big|_{\text{target}})$ clamps the target upward to the minimum gain bound so the optimiser never recommends a device with gain below your floor.
...
5. **Set Bound** - minimum acceptable $g_m/g_{\text{ds}}$ constraint. The optimizer won't accept solutions below this.
...
| Device | L | $I_D/W$ (µA/µm) | $I_D$ (µA) | $W_{\text{unit}}$ (µm) | $f_T$ (GHz) |
|:--|:--|:--|:--|:--|:--|
...
> - $R_{\text{on}}$ of the output transistors and any information about the Non-Linear settling information(sets non-linear RC settling speed)
> - Deadzone bias voltages $V_{\text{DZP}}$ / $V_{\text{DZN}}$ (the small-signal settling window)
> - Whether the RC phase even completes within the clock half-cycle
> - Worst-case SS-corner behaviour for any of the above
>
> These unknowns require additional SPICE iterations which is the gap the $R_{\text{on}}/g_m$ methodology closes.
...
(LV_NMOS: $W = 0.3\,\mu\text{m}$, $L = 3.0\,\mu\text{m}$, $m = 4$; LV_PMOS: $W = 0.15\,\mu\text{m}$, $L = 0.25\,\mu\text{m}$, $m = 4$; $I_{q,\text{replica}} = 0.5\,\mu\text{A}$, $I_{q,\text{signal}} = 2.0\,\mu\text{A}$)
...
|:--|:--|:--|:--|
| **Fig 11** : $V_{\text{out}}$ | `sv_g` = Vout(t) from file | Direct read from transient file | Whether RC phase completes in time |
| **Fig 12** : Log error | `err_g = (Vout − Vfinal)/Vfinal` | Relative error on log scale; `curve_fit` extracts τ | τ of the RC phase (driven by $R_{\text{on}}$) - unknown to gm/Id |
| **Fig 13** : $I_{\text{out}}$ | `sm_g` = branch current × 1e6 | Raw current waveform in µA | $I_{\text{peak}}$ magnitude — set by $R_{\text{on}}$, invisible to gm/Id |
| **Fig 14** : $dV_{\text{out}}/dt$ | `dvdt_g = np.gradient(sv_g, st_us_g)` | Numerical derivative = slew rate | Slew rate peak = $I_{\text{peak}}/C_L$ — depends on $R_{\text{on}}$ |

> The gm/Id design settles correctly, but the designer had no pre-simulation visibility into
> $R_{\text{on}}$, the deadzone bias ranges, or the SS-corner worst case
> all of which are characterised pre-simulation in the $R_{\text{on}}/g_m$ section that follows.
...
Figures 10–14 confirmed that the gm/Id-sized IBA settles correctly at **250 ns settling time** (UGB = 9.55 MHz, $\omega_{\text{cl}}$ = 20 Mrad/s).
But three critical quantities remained unknown until post-simulation:

| Unknown | What it governs | How discovered in gm/Id flow |
|:--|:--|:--|
| $R_{\text{on}}$ of NMOS/PMOS | RC settling time constant $\tau_{\text{ls}} = R_{\text{on}}C_L$ (the Fig 13 current spike) | Only after transient SPICE |
| Deadzone voltages $V_{\text{DZP}}$, $V_{\text{DZN}}$ | Define small-signal settling window; too wide = instability | Only after iterative corner sweeps |
| SS-corner $R_{\text{on}}$ degradation | Worst-case non-linear settling | Only after running SS corner sim |

The **$R_{\text{on}}/g_m$ methodology** below resolves all three simultaneously,
pre-simulation, using the LUT plots generated in the characterisation section.

---


# 6. Design of Inverter-Based Amplifier using the $R_{\text{on}}/g_m$ Methodology
---

#### Objective
To design an inverter-based amplifier targeting a **$T_{\text{settle}} = 250\,\text{ns}$** settling time (UGB = 9.55 MHz, $\omega_{\text{cl}}$ = 20 Mrad/s), while ensuring effective settling behavior and low power consumption through the $R_{\text{on}}/g_m$ methodology.
...
- With $C_{\text{load}} = 667\,\text{fF}$ and $T_{\text{settle}} = 250\,\text{ns}$, we target:
...
- The **worst-case $R_{\text{on}}$ occurs at the SS corner**, which drives our design:

  $$\frac{R_{\text{on}}}{G_m} = \frac{1\,\text{k}\Omega}{40\,\mu\text{S}} = 25 \times 10^6, \qquad
  \frac{R_{\text{on}}}{g_{\text{mn}}} = \frac{R_{\text{on}}}{g_{\text{mp}}} = \frac{1\,\text{k}\Omega}{20\,\mu\text{S}} = 50 \times 10^6$$

- Thus, we design at:

  $$R_{\text{on}}/g_m = 50 \times 10^6 \quad (\text{per device SS corner})$$

---

### Pre-Simulation Predictions from $R_{\text{on}}/g_m$ Plots for NMOS

| Quantity | Source | Pre-Sim Value |
|---|---|---|
| $V_{\text{bias},\text{TT}}$ | $V_{\text{bias}}$ vs $R_{\text{on}}/g_m$ | 0.39 V |
| $V_{\text{bias},\text{SS}}$ | $V_{\text{bias}}$ vs $R_{\text{on}}/g_m$ | 0.42–0.43 V |
| $V_{\text{bias},\text{FF}}$ | $V_{\text{bias}}$ vs $R_{\text{on}}/g_m$ | 0.35–0.36 V |
| $g_m/I_D$ | $g_m/I_D$ vs $R_{\text{on}}/g_m$ | $\approx 10$ |
| $I_{\text{peak}}$ | $I_{\text{peak}}$ vs $R_{\text{on}}/g_m$ | $\approx 14\,\mu\text{A}$ |
...
- The predicted **peak current**:
  - Pre-simulation: $I_{\text{peak}} \approx 14\,\mu\text{A}$
  - Post-simulation: $\approx 12\,\mu\text{A}$  
  → within **~15% accuracy**

- The predicted **deadzone bias voltages**:
  - $V_{\text{DZN}}$: Pre-simulation $\approx 0.39\,\text{V}$ (TT) — Post-simulation within $\pm50\,\text{mV}$
  - $V_{\text{DZP}}$: Pre-simulation $\approx 1.11\,\text{V}$ (TT) — Post-simulation within $\pm50\,\text{mV}$  
...
- This reveals a **fundamental one-way asymmetry** between the two methodologies:
  - $R_{\text{on}}/g_m$: DC bias range, corner spread, $g_m/I_D$ operating point, and $I_{\text{peak}}$ are all read **directly from plots before simulation**.
  ...
  The $R_{\text{on}}/g_m$ methodology strictly subsumes $g_m/I_D$; the reverse is impossible.
...
## 6.1 $R_{\text{on}}/G_m$ Design Helper
---

> **Design Step:** The calculation for the unit cell above fixed $R_{\text{on}}/g_m = 50\times10^6$ and $I_{\text{bias}} = 0.5\,\mu\text{A}$, confirmed by reading Plot 1 and Plot 5.
> Use this helper to search the pre-characterised SPICE LUT for the exact $(W, L, V_{\text{bias}})$ that meets these targets at your chosen corner.
...
whose on-resistance $R_{\text{on}}$ sets the RC settling time constant $\tau_{\text{ls}} = R_{\text{on}}C_L$.
...
$$\tau_{\text{ls}} = R_{\text{on}}C_L, \qquad \tau_{\text{ss}} = \frac{C_L}{g_m}$$

A single $R_{\text{on}}/G_m$ target constrains both time constants simultaneously
$R_{\text{on}}$ bounds the large-signal settling budget and $G_m$ bounds the small-signal
settling bandwidth without requiring separate iterations.

The LUT is pre-characterised across different $V_{\text{GS}}$, $V_{\text{DS}}$ (So that user can design the $R_{\text{on}}/G_m$ based on the input swing eg., Lets say we want to design the dynamic amplifier for an input swing of VDD/2 or Vdd/4 and so on), channel length $L$,
and process corner. For a specified bias condition $(V_{\text{GS}}, V_{\text{DS}}, \text{corner})$,
...
$(I_D,\; (R_{\text{on}}/G_m)_{\text{unit}},\; L)$:

$$\mathcal{L} = \left(\frac{I_D - \hat{I}_D}{\sigma_{I}}\right)^{\!2}
              + \left(\frac{(R_{\text{on}}/G_m)_{\text{unit}} - \widehat{(R_{\text{on}}/G_m)}_{\text{unit}}}{\sigma_{\text{RG}}}\right)^{\!2}
              + \left(\frac{L - \hat{L}}{\sigma_{L}}\right)^{\!2}$$

$\sigma$-normalisation is essential: $I_D$, $(R_{\text{on}}/G_m)_{\text{unit}}$, and $L$ span
...
This tool helps the user to size the dynamic amplifier based on $R_{\text{on}}/G_m$ based design methodology.
...
4. **Select VDS / VSD** → the drain to source voltage. Use small values (e.g., 0.05 V) to characterise the linear-region $R_{\text{on}}$.
5. **Set i-sweep** → the bias current value. Use the slider or type directly in the text box. This corresponds to $I_{\text{bias}}$ in the replica path.
6. **Set Ron/Gm** → your target $R_{\text{on}}/g_m$ value in Ω·(A/V)⁻¹. Read your required value from the design methodology plots above.
7. **Set length** → target channel length. Longer channels give higher $R_{\text{on}}$ and lower $g_{\text{ds}}$; shorter channels are faster.
...
We design it for **SS corner** (worst-case $R_{\text{on}}$):

- Target $R_{\text{on}}/g_m = 50\times10^6$, $I_{\text{bias}} = 0.5\,\mu\text{A}$  

- Since there is **no LUT data for $I_{\text{sweep}} = 0.5\,\mu$A**, we scale from the minimum available data:
  - Available: $I_{\text{sweep}} = 5\,\mu$A  
  - Required: $I_{\text{sweep}} = 0.5\,\mu$A  
  - Scaling factor = **0.1× → Width scales by 0.1**

- **$V_{\text{bias}}$ is invariant under this scaling.**  
  $V_{\text{bias}}$ sets $V_{\text{GS}}$ of the bias-path device. In both weak and strong inversion,
  $V_{\text{GS}}$ depends only on the current density $I_D/W$, not on absolute current:
  - Weak inversion: $V_{\text{GS}} = V_{\text{th}} + nU_T\ln\!\left(\dfrac{I_D \cdot L}{I_0 \cdot W}\right)$ only $I_D/W$ appears.
  - Strong inversion: $V_{\text{GS}} - V_{\text{th}} = \sqrt{\dfrac{2I_D L}{\mu C_{\text{ox}} W}}$ only $I_D/W$ appears.

  Since $I_D$ and $W$ are scaled by the same factor $k$, the ratio $I_D/W$ is unchanged,
  the inversion level is unchanged, and $V_{\text{GS}} = V_{\text{bias}}$ is unchanged.
...
### Reference from Ron/gm Helper ($I_{\text{sweep}} = 5\,\mu$A)
...
> Unlike gm/Id, this method allows **direct scaling across current levels** while preserving $R_{\text{on}}/g_m$, enabling reliable sizing even when LUT data is unavailable at the target bias.

---


# 6.2 Inverter Based Amplifier design Results: $R_{\text{on}}/g_m$ Methodology

## Schematic
Schematic of the inverter-based amplifier with capacitive feedback, designed using the $R_{\text{on}}/g_m$ methodology:
...
*Fig. 15: Inverter-based amplifier in capacitive feedback configuration designed using the $R_{\text{on}}/g_m$ methodology.*


## 6.3 Results from the IBA designed using **$R_{\text{on}}/g_m$** design methodology

The four figures below are generated from `INV_OTA_rongm.txt`, a transient simulation of the IBA sized using the $R_{\text{on}}/g_m$ methodology.

(LV_NMOS: $W = 0.5\,\mu\text{m}$, $L = 3.0\,\mu\text{m}$, $m = 4$; LV_PMOS: $W = 0.15\,\mu\text{m}$, $L = 0.2\,\mu\text{m}$, $m = 4$; $I_{q,\text{replica}} = 0.5\,\mu\text{A}$, $I_{q,\text{signal}} = 2.0\,\mu\text{A}$)
...
It is observed that the error envelope exhibits **two distinct settling regimes**: an initial rapid decay with a **steep negative slope (500–550 ns)** attributable to **large-signal nonlinear operation**, followed by a gradual exponential decay with a **comparatively smaller slope**, characteristic of **small-signal linear settling**. It is worth noting that the boundary between
these two regimes, and consequently the proportion of the settling window occupied by the nonlinear phase, **determined beforehand from $R_{\text{on}}/g_m$ design methodology.** This information was ready with users before simulation analysis. This constitutes a
...
Both methodologies successfully meet the 9.55 MHz UGB bandwidth specification: the $g_m/I_D$ design (NMOS: $W = 0.3\,\mu$m, $L = 3\,\mu$m, $m = 4$), (PMOS: $W = 0.15\,\mu$m, $L = 0.25\,\mu$m, $m = 4$) and the $R_{\text{on}}/g_m$ design (NMOS: $W = 0.5\,\mu$m, $L = 3\,\mu$m, $m = 4$), (PMOS: $W = 0.15\,\mu$m, $L = 0.2\,\mu$m, $m = 4$) both settle correctly, as confirmed by Figures 10–14 and 15–19.

The critical difference lies in **what the designer knows before running the transient simulation.** With the $g_m/I_D$ approach, the on-resistance $R_{\text{on}}$ of the output devices, the deadzone voltage ranges $V_{\text{DZP}}/V_{\text{DZN}}$, and the worst-case SS-corner behaviour are all unknown at design entry, they are discovered only after running additional SPICE sweeps. With the $R_{\text{on}}/g_m$ approach, all three are read directly from the LUT plots at the point of sizing.

This distinction becomes especially significant for dynamic amplifier design, where settling involves two sequential phases. The $g_m/I_D$ methodology was not designed with this two-phase behaviour in mind, and as a result leaves the large-signal RC phase uncharacterised. The $R_{\text{on}}/g_m$ methodology treats both phases as main design variables from the outset, reducing the number of design iterations required to reach a corner-robust, fully specified operating point.

<hr>

### Limitations

- **No physical layout:** Device sizes and bias voltages are schematic-level. Silicon area and post-layout parasitic impact on $R_{\text{on}}/g_m$ are not verified yet.
...
<p align="justify" style="text-align: justify; margin-bottom: 20px;">
<a href="https://www.linkedin.com/in/suyajnaa-jagannath-13915032b/" target="_blank"><img src="https://raw.githubusercontent.com/chennakeshavadasa/CAC/main/DEMO/assets/Suyajnaa_Pic.jpeg?v=5" align="left" style="width: 120px; margin-right: 15px; border-radius: 8px;" /></a>
...
<p align="justify" style="text-align: justify; margin-bottom: 20px;">
<a href="https://www.linkedin.com/in/runpeng-gao-842a44263/" target="_blank"><img src="https://raw.githubusercontent.com/chennakeshavadasa/CAC/main/DEMO/assets/Runpeng_Gao_pic.jpeg?v=5" align="left" style="width: 120px; margin-right: 15px; border-radius: 8px;" /></a>
...
<p align="justify" style="text-align: justify; margin-bottom: 20px;">
<a href="https://www.linkedin.com/in/praveen-kumar-venkatachala-5b816b36/" target="_blank"><img src="https://raw.githubusercontent.com/chennakeshavadasa/CAC/main/DEMO/assets/Praveen_Kumar_pic.jpg?v=5" align="left" style="width: 120px; margin-right: 15px; border-radius: 8px;" /></a>
